...
(function($, document, window, undefined) {
    ...
    var pluginName = 'asDatepicker',
        defaults = {
            ...
            tplContent: function() {
                var _tpl =  '<div class="namespace-box">' +
                    ...
                    '<i class="pc-cancel">清空</i>' +
                    '<i class="pc-save">确定</i>' +
                    '</div>' +
                    '</div>';
                    if(this.showButton){
                        _tpl += '<div class="recent-box">' +
                        '<button class="recent" data-day="1" type="button">今天</button>' +
                        '<button class="recent" data-day="2" data-count="1" type="button">昨天</button>' +
                        '<button class="recent" data-day="3" type="button">最近3天</button>' +
                        '<button class="recent" data-day="7" type="button">最近7天</button>' +
                        '<button class="recent" data-day="month" type="button">本月</button>' +
                        '</div>'
                    }
                 _tpl +='</div>';
                 return _tpl;
    ...
    Plugin.prototype = {
        ...
        _initStatus: function(displayMode) {
            if (displayMode === 'inline') {
                ...
                $doc.on('click', '.recent', $.proxy(this._recentn, this));
                $doc.on('click', '.pc-save', $.proxy(this._pcsave, this));
                $doc.on('click', '.pc-cancel', $.proxy(this._pccancel, this));
        ...
        _initDate: function() {
            ...
            if (this.mode === 'multiple') {
                ...
                for (var i = 0; i < arr.length; i++) {
                    var dd = new Date(arr[i]);
                    dd.setHours(0, 0, 0, 0);
                    dd = Date.parse(dd);
                    datearr.push(dd);
                }
                this._date.selectedDate = datearr;
        ...
        _generateDays: function(index) {
            ...
            for (var j = 0; j < 42; j++) {
                ...
                if (j < daysFromPrevMonth) {
                    //prev month days
                    className = this.namespace + '_prevMonth';
                    ...
                    //next month days
                    className = this.namespace + '_nextMonth';
            ...
            switch (mode) {
                ...
                case 'multiple':
                    for (var i = 0; i < this._date.selectedDate.length; i++) {
                        if (Date.parse(currentDate) === Date.parse(new Date(this._date.selectedDate[i]))) {
                            active = true;
                        }
                    }
                    break;
        ...
        _recentn: function(e) {
            var arr = [],
                datearr = [];
            var n = e.target.dataset.day;
            var count = e.target.dataset.count;
            if (n == 'month') {
                var _thismonths = new Date().setDate(1);
                var _thismonthe = new Date().setDate(new Date().getDate());
                n = (_thismonthe - _thismonths) / (1000 * 24 * 3600)+1;
            }
            if (count) {
                var ddate = new Date().setHours(0, 0, 0, 0) - count * 3600 * 24 * 1000;
                var ss = this._formatDate(ddate, this.outputFormat);
                arr.push(ss);
                datearr.push(ddate);
            } else {
                for (var i = 0; i < n; i++) {
                    var ddate = new Date().setHours(0, 0, 0, 0) - i * 3600 * 24 * 1000;
                    var ss = this._formatDate(ddate, this.outputFormat);
                    arr.push(ss);
                    datearr.push(ddate);
                }
            ...
            this._updateDate(0);
            this._manageViews(0);
            // this.$el.css('width', 86 * arr.length + 'px');
        },
        _pcsave: function(e) {
        ...
        _click: function(e) {
            ...
            if ($target.closest(this.$inputIcon).length === 0 && $target.closest(this.$picker).length === 0 && $target.closest(this.$el).length === 0 && this.options.alwaysShow === false) {
                ...
                if (_targetSpan.length === 1) {
                    ...
                    if (!_targetSpan.hasClass(this.namespace + '_untouchable') && _targetSpan.parent('.' + this.namespace + '-head').length !== 1) {
                        if (!_targetSpan.hasClass(this.namespace + '_prevMonth') && !_targetSpan.hasClass(this.namespace + '_nextMonth')) {
                            this._changeValue(_targetSpan, j);
                        } else {
                            this._changeValue(_targetSpan, j, _target);
                        }
        ...
        _changeValue: function(target, i, tar) {
            var newVal = '',
                newDate = '',
                self = this;
            switch (this.views[i]) {
                ...
                case 'days':
                    newVal = parseInt(target.text(), 10);
                    console.log(tar, $(tar).find('span'))
                    if (!tar) {
                        newDate = new Date(this._date.currentYear[i], this._date.currentMonth[i], newVal, 0, 0, 0, 0);
                    } else if ($(tar).find('span').hasClass(this.namespace + '_prevMonth')) {
                        var curr = this._date.currentMonth[i] - 1;
                        newDate = new Date(this._date.currentYear[i], curr, newVal, 0, 0, 0, 0);
                    } else if ($(tar).find('span').hasClass(this.namespace + '_nextMonth')) {
                        var curr = this._date.currentMonth[i] + 1;
                        newDate = new Date(this._date.currentYear[i], curr, newVal, 0, 0, 0, 0);
                    }
                    // newDate = new Date(this._date.currentYear[i], this._date.currentMonth[i], newVal, 0, 0, 0, 0);
        ...
        _setValue: function() {
            switch (this.mode) {
                ...
                case 'multiple':
                    ...
                    this.$el.val(val);
                    // this.$el.css('width', 86 * this._date.selectedDate.length + 'px')
                    break;
            }
